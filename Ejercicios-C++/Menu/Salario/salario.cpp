...
int main() {
  ...
  // Las tarifas y retenciones como no varian son constantes
  const float TARIFA_NORMAL = 350.00f;
  const float TARIFA_EXTRA = 1.5 * TARIFA_NORMAL; 
  const float RETENCION_1 = .12f, RETENCION_2 = .18f;
  ...
  cout << "Retenciones: $" << fixed << setprecision(2) << retenciones << endl;
  cout << "Menos retenciones, su sueldo neto es de: $" << fixed << setprecision(2) << sueldo_neto << endl;
