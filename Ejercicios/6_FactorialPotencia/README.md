...
Regresamos a la línea donde declaramos las variables iniciales, y añadimos dos nuevas: **pot** que será el resultado y **expo** que es el número al que vamos a elevar el número introducido.
```C++
int n, i, suma, pot, expo;
```

En la segunda parte agregamos nuevamente un `printf` para mostrar el mensaje de `"Introduce un exponente"` y un `scanf` para almacenar el valor del exponente con `"%d", &expo` como parámetros.
...
Después del primer bucle for, inicializamos la variable **pot** de la misma manera que lo hicimos con **suma**, igualandola al número del usuario **n**, esto para tener al número como punto de partida.

```C++
pot = n;
```

Realizamos otro bucle `for`, donde vamos a iterar con la variable **j** desde 1 (**_j = 1_**), mientras sea menor al valor del exponente introducido por el usuario (**_j < expo_**) y vamos a incrementar de 1 en 1 la variable (**_j++_**), en cada iteración vamos almacenar en la variable **pot** el valor de ella misma multiplicado por **n**.
